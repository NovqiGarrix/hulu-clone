...
const Card = ({ movie }: any) => {
  ...
  useEffect(() => {
    const fetchDetailMovie = async () => {
      const { data } = await axios.get(
        `https://api.themoviedb.org/3/movie/${movie?.id}?api_key=${API_KEY_IMDB}&language=en-US`
      );
      return data;
    };
  ...
  return (
    <a
      ...
      target="_blank"
      key={movieDetail?.id}
      className="group cursor-pointer sm:hover:scale-105 duration-150 transform hover:z-10 mb-2 w-full h-full"
    >
      <div className="w-52">
        <Image
          src={
            `${IMAGE_BASE_URL}/${movieDetail?.poster_path}` ||
            `https://image.freepik.com/free-photo/vintage-wallpaper-background_53876-25244.jpg`
          }
          width={1080}
          height={1920}
          layout="responsive"
      ...
      <div className="text-gray-300 group-hover:text-white group-hover:font-semibold">
        ...
        <p className="text-sm flex items-center mb-3">
          ...
          <StarIcon className="w-3 h-3 ml-1 mr-2" /> from{" "}
